 ...
 * talks to its own local database instead of erlbutt.  (That database is
 * ~/.silkpurse unless ssb_appname says otherwise -- NOT ~/.ssb, which
 * nothing here touches by default.)
 *
 * Presence of a SECRET is what selects remote mode -- not the address.
 * ERLBUTT_ADDR on its own is ignored and you stay local, which looks
 * exactly like erlbutt mode failing to connect.  No secret (no env var,
 * no erlbutt block) => local mode, exactly as before.
 */
