...
const pages = ['/public', '/private', '/mentions', '/all', '/participating', '/profile']

module.exports = function (api) {
  const i18n = api.intl.sync.i18n
  return {
    'app.html.search': function (setView) {
      ...
      electron.ipcRenderer.on('search-unavailable', () => {
        searchBox.setAttribute('disabled', true)
        searchBox.setAttribute('placeholder', i18n('Loading search data...'))
        setTimeout(() => {
          electron.ipcRenderer.send('is-search-available')
        }, 5000)
      })

      electron.ipcRenderer.on('search-available', () => {
        searchBox.removeAttribute('disabled')
        searchBox.setAttribute('placeholder', i18n('word, @key, #channel'))
      })
