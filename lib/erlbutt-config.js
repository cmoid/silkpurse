 ...
 * inherits no shell environment, so an env-only setting means a
 * double-clicked build finds nothing and cannot start.
 *
 * The SECRET is what makes this resolvable -- not the address.
 * ERLBUTT_ADDR on its own returns null here, because an address with no
 * identity is not something we can connect as.
 *
 * Returning null means "not configured", and the caller treats that as
 * fatal: there is no embedded server to fall back to any more, so an
 * unconfigured Silkpurse says what it needs and quits rather than
 * starting into a state that cannot work.
 *
 * This runs BEFORE ssb-config/inject, so it is handed rc's raw sources
 * rather than a finished config -- see setupContext in index.js for why
 * that ordering keeps a useless keypair out of the data directory.
 */
