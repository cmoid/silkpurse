...
// How far back a BRAND NEW index reaches on its first build.
//
// Asking for everything is not survivable on a large store. A source is
// served inside one 45-second muxrpc call — the sends have to be
// serialised on the connection because each one advances the nonce — so a
// request that takes longer than that kills the connection it arrived on.
// The client then reconnects, finds its index still empty, asks for
// everything again, and never gets anywhere. Against a converted corpus
// (2.5M messages, ~470k posts, one file read each) that is what happens.
//
// So a new index starts from a window rather than from zero: recent
// history is searchable within seconds, and the part that cannot be
// served in one call is simply not claimed. Deliberately incomplete, and
// visibly so, rather than complete-in-principle and broken in practice.
//
// The real fix is a bounded request that the client pages through; when
// that exists this becomes a plain zero again.
const FIRST_INDEX_DAYS = 90;
...
function startSearchIndexing(sbot, ssbConfig) {
  ...
  const loadOrCreateSqlite = async () => {
    ...
    // Whether there is anything to search RIGHT NOW, which is not the same
    // as where indexing should start from — and stops being the same the
    // moment a new index is seeded with a non-zero watermark below.
    const hasPriorWork = lastIndexedTimestamp > 0;

    if (lastIndexedTimestamp === 0) {
      lastIndexedTimestamp = Date.now() - FIRST_INDEX_DAYS * 86400000;
      serverlog(
        `[SQLITE] new index: seeding from ${
          new Date(lastIndexedTimestamp).toISOString()
        } (last ${FIRST_INDEX_DAYS} days). Older messages are not indexed.`,
      );
    }
