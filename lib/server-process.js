...
// LAN discovery (ssb-lan) is what surfaces the local erlbutt node as a
// discoverable peer.  It was removed in f669bd42 because its UDP transport
// (dgram-broadcast, formerly broadcast-stream) creates a socket with no
// 'error' handler: on macOS a stray recvmsg ENOTCONN — the one-shot error
// macOS raises after an ICMP unreachable — becomes an uncaught exception
// that wedges the server process.  Wrap the transport so every socket it
// hands back has an error handler that absorbs the (non-fatal) error; the
// socket keeps receiving.  This MUST run before ssb-lan is required below.
(function guardLanBroadcastSocket() {
  try {
    const id = require.resolve("dgram-broadcast");
    const createStream = require(id);
    require.cache[id].exports = function guardedCreateStream(...args) {
      const socket = createStream(...args);
      socket.on("error", (err) => {
        console.error("[ssb-lan] broadcast socket error (ignored):", err.message);
      });
      return socket;
    };
  } catch (err) {
    console.error("[ssb-lan] could not guard broadcast socket:", err.message);
  }
})();
