...
module.exports = function (api) {
  ...
  return {
    'page.html.render': function profile (id) {
        ...
        h('div.image', [
          ...
          h('div.lastActivity',
            computed(lastActivity,
              msg => `${i18n('Last activity')}: ${timestamp(msg, false)}`))]),
        h('div.main', [
          h('div.title', [
            h('h1', [name]),
            h('div.meta', [
  ...
  // A note at the foot of the timeline when this node deliberately does not
  // hold the start of the feed.
  //
  // It cannot come from the timeline itself: the archive boundary is a
  // message of type `archive`, and the roots stream only carries posts, so
  // scrolling to the bottom shows the oldest post this node has with no
  // indication that anything precedes it. Asking archives.history instead
  // answers from the floor, which is recorded the moment the boundary is
  // adopted — before the boundary message itself has even replicated.
  //
  // Silent in local mode, where the method does not exist and there is
  // nothing to say: an embedded server holds whole feeds.
  function archiveFooter (feedId, feed) {
    const holder = h('div.ArchiveFooter')
    if (!api.sbot.async.archivesHistory) return holder

    api.sbot.async.archivesHistory(feedId, (err, res) => {
      if (err || !res || !res.skipped) return
      holder.appendChild(renderArchiveFooter(feedId, res.skipped, holder, feed))
    })
    return holder
  }

  function renderArchiveFooter (feedId, skipped, holder, feed) {
    const status = Value(null)

    const fetchButton = h('button.archive-fetch', {
      'ev-click': () => archiveFetch.start({
        api,
        i18n,
        feedId,
        status,
        element: holder,
        // The timeline's backlog was fetched before the import, so it
        // still ends where the floor used to be. Restored history only
        // appears once that query runs again.
        onImported: () => feed.reload()
      })
    }, i18n('Fetch earlier history'))

    return h('div.ArchiveBoundary', [
      h('div.archive-rule'),
      h('div.archive-label', [
        h('strong', i18n('Earlier history archived')),
        h('div.archive-detail', summary.fromSkipped(skipped, i18n)),
        h('div.archive-action', [
          h('span', [status]),
          whileEmpty(status, fetchButton)
        ])
      ])
    ])
  }

  // Mutant listeners are not called on bind, so the current value has to be
  // applied by hand or the button never appears at all.
  function whileEmpty (obs, element) {
    const holder = h('span')
    const update = (value) => {
      holder.innerHTML = ''
