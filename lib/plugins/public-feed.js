...
exports.init = function (ssb) {
  ...
  return {
    ...
    roots: function ({ reverse, limit, resume }) {
      ...
      const opts = { reverse, old: true, awaitReady: false }
      if (resume) {
...
function getFilterResult (msg, { ssb }, cb) {
  ssb.patchwork.contacts.isFollowing({ source: ssb.id, dest: msg.value.author }, (err, following) => {
    ...
    ssb.patchwork.subscriptions2.get({ id: ssb.id }, (err, subscriptions) => {
      ...
      const hasChannel = !!msg.value.content.channel
      const matchesChannel = (type !== 'channel' && checkChannel(subscriptions, msg.value.content.channel))
      const matchingTags = getMatchingTags(subscriptions, msg.value.content.mentions)
      const isYours = msg.value.author === ssb.id
      const mentionsYou = getMentionsYou([ssb.id], msg.value.content.mentions)
...
function getMentionsYou (ids, mentions) {
  if (Array.isArray(mentions)) {
    return mentions.some((mention) =>
      mention && typeof mention.link === 'string' && ids.includes(mention.link)
    )
  }
}

function checkReplyForcesDisplay (item) {
  const filterResult = item.filterResult || {}
  const matchesTags = filterResult.matchingTags && !!filterResult.matchingTags.length
