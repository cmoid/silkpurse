...
module.exports = function (config) {
  const { api, use } = Plug(require('./plug-keys'))
  use(() => ({ 'config.sync.load': () => config }))
  use(() => ({ 'app.navigate': navigate }))
  use(require('./depject'))
  use(require('patch-settings'))
  ...
  // context menus are handled on the server. however, we need to know some
  // information about the element under the cursor at the time of clicking.
  // Sadly, electron doesn't seem to allow us to attach arbitrary information to
  // an event and have it be transferred to the server directly. so we need to
  // transfer that information "out of band" via sendSync.
  // Note that invoke() might cause race conditions here IFF messages can change
  // order in the queue between Renderer and Server, or if the IPC and document
  // events use different queues. I'm not sure whether that's the case, but I
  // haven't observed this happening yet.
  document.addEventListener('contextmenu', (ev) => {
    let element = document.elementFromPoint(ev.x, ev.y)
    while (element && !element.msg) {
      element = element.parentNode
    }
    // we only send the id, because that's cheap
    // if the server actually wants to copy the text, it will
    // ask us (see "copy-message-text" handler below)
    electron.ipcRenderer.invoke('context-menu-info', {
      msg: element?.msg || null
    })
  })

  // for when the server process asks us to please copy a message text to
  // clipboard needed for context menu popups, because we want to call the
  // getMessage only *if* the text is actually to be copied
  electron.ipcRenderer.on('copy-message-text', (ev, key) => {
    getMessageText(key, (err, value) => {
      electron.clipboard.writeText(
        !err ? value : `Error while retrieving message ${key}:\n${err}`
      )
    })
  })
