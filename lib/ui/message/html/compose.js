...
module.exports = function (api) {
  ...
  return {
    'message.html.compose': function ({ shrink = true, isPrivate, participants, meta, hooks, prepublish, placeholder = 'Write a message', draftKey }, cb) {
      const files = []
      const filesById = {}
      ...
      // Drafts are scoped to the logged-in feed. localStorage is per-origin,
      // not per-identity, so an unscoped key let a draft written as one
      // identity reappear as another — a draft composed as EarlButt showed up
      // when running as Moid, reading as an unrendered message from EarlButt.
      const draftId = draftKey && `${api.keys.sync.id()}.${draftKey}`

      if (draftId) {
        const draft = window.localStorage[`patchwork.drafts.${draftId}`]
        if (draft) {
          textArea.value = draft
        }
        const draftCW = window.localStorage[`patchwork.drafts.contentWarning.${draftId}`]
        if (draftCW) {
      ...
      function save () {
        ...
        if (draftId) {
          if (!textArea.value) {
            delete window.localStorage[`patchwork.drafts.${draftId}`]
          } else {
            window.localStorage[`patchwork.drafts.${draftId}`] = textArea.value
          }
          if (!contentWarningInput.value) {
            delete window.localStorage[`patchwork.drafts.contentWarning.${draftId}`]
          } else {
            window.localStorage[`patchwork.drafts.contentWarning.${draftId}`] = contentWarningInput.value
          }
        }
      }
