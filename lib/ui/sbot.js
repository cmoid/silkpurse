...
module.exports = function (api) {
  ...
  return {
    ...
    // erlbutt only. In local mode the embedded server has no archives
    // namespace, and it holds every feed whole, so there is nothing to
    // ask for — the renderer does not offer the call in that mode.
    'sbot.async.archivesHistory': rec.async(function (feedId, cb) {
      if (!sbot.archives) return cb(new Error('archives not supported here'))
      sbot.archives.history(feedId, cb)
    }),

    'sbot.async.archivesFetch': rec.async(function (feedId, cb) {
      if (!sbot.archives) return cb(new Error('archives not supported here'))
      sbot.archives.fetch(feedId, cb)
    }),
